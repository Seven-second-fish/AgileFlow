...
用户一次提交多条缺陷或验收反馈时，统一进入 `/af-revise`，先归因再修改：

批量、跨层、阻塞、重开或高风险缺陷须按
[bug 模板](../templates/bug.md)创建 `atlas/bugs/README.md`；单个且当场闭环的轻微
`/af-fix` 不创建。Bug 状态以总表为唯一权威。

| 归因 | 判定 | 后续 |
|------|------|------|
| 实现偏离 | 已有 AC/预期清楚，代码没有做到 | 在 revise 中集中修复并回归 |
| solution/model 问题 | AC 不变，但设计、契约或模型不支持 | L1，同步上游后修复 |
| 需求问题 | AC 缺失、矛盾，或用户明确修改既有规则 | L2，先改 REQ 和影响分析 |
| 新需求 | 既有 REQ/AC 从未覆盖的新行为 | 从缺陷批次拆出，进入 `/af-req` |

混合批次逐条分类，不能因为其中一条需求问题就把全部缺陷判成 L2。`/af-test`
用于执行验收与记录结果，不是接收用户缺陷清单的入口。若归因后超出 revise 边界，
在实施前升级完整 Flow。
...
- 需求有、实现没做 → `实现遗漏`
- 需求有、实现做错 → `实现错误`
- 需求没有但属于现有功能应有行为 → `需求漏记`，先补 REQ/AC
- 需求没有且是用户现在新增的能力 → `新需求`，拆出 `/af-req`

修复顺序固定为：**需求对账 → 必要时修订上游 → 补失败测试/复现 → 修代码 →
受影响 REQ 回归 → 更新 Bug 状态**。补测试属于修 Bug 本身，不另切 `/af-ut`。
关闭前运行：

```bash
agileflow validate --only bugs --root .
```
...
3. 归因与边界：逐项判实现、solution/model、需求或新需求；越界则先升级
4. 声明影响面：列出将修改的文件（已有文档 + 代码）
5. 执行：更新已有文档对应段落 + 修改代码
6. 验证：编译 + 相关测试 + 文档与代码一致性自检
7. 收尾：`✅ revise 完成：{改了什么}（REQ/sol/code 已同步）`
